...
export function makeMoveMaxN(gameState: IGameState, _depth?: number): IGameState {
    ...
    for (let i = 1, ie = nextGSs.length; i < ie; ++i) {
        ...
        const nextScore = maxN(nextGSs[i], depth - 1);
...
/** _Important_: The parameter `opponentsBestScore` is used for the recursion
 * internally. **Do not set it**, when calling the function externally! */
function maxN(gs: IGameState, depth: number, opponentsBestScore = 0): TPlayerScore {
    if (isGameOver(gs) || depth <= 0) return calcPlayerScore(gs);
    
    const player = gs.player;
    const maxScore = MAX_SCORE - opponentsBestScore; // immediate & shallow pruning
