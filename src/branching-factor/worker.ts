...
import { getNextGameStates, IGameState } from 'chameleon-chess-logic';
 ...
 * ```
 */
type TInput  = { gameState: IGameState, depth: number };
...
function calcBranchingFactor(gameState: IGameState, depth: number): number {
    const nextGSs = getNextGameStates(gameState);

    if (depth <= 1) return nextGSs.length;
